...
- The examples use _JSON_ - despite its [flaws](http://github.com/galliaproject/gallia-docs/json) - because of its ubiquity as a notation
...
<div style="text-align:center"><img src="./images/dependencies.png" alt="core dependency graph"></div>
 ...
 Note that there exists at this time two other modules besides _core_, with additional dependencies of their own: _gallia-mongodb_ and _gallia-spark_ (discussed further down)
...
- This example makes use of `.toLocalDateFromIso()` and `.thn()` from our `import aptus._` above (see [docs](http://github.com/galliaproject/gallia-docs/aptus))
...
### Path referencing
Paths can be referenced conveniently via the "pipe+greater-than" (`|>`) [notation](http://github.com/galliaproject/gallia-docs/tasks#t210127123739) :
```scala
"""{"parent": {"foo": "value"}}"""
...
Renaming can be expressed conveniently via the "tilde+greater-than" (`~>`) [notation](http://github.com/galliaproject/gallia-docs/tasks#t210127123739) :
...
Note that [unpivoting](http://github.com/galliaproject/gallia-docs/tasks#t210120171258) isn't available, but scheduled
...
This alone would deserve its own article, but in the meantime the [DbNsfp](#210121135252) example highlights a number of interesting such cases.

The opposite operation (_flattening_) is [scheduled](http://github.com/galliaproject/gallia-docs/tasks#t210121160211) .
...
<a name="210121153220"></a>
 ...
 - Reading and writing to/from [HDFS](http://github.com/galliaproject/gallia-docs/tasks#t210120153618) and [S3](http://github.com/galliaproject/gallia-docs/tasks#t210120153619) isn't actually ready, neither is using [lzo](http://github.com/galliaproject/gallia-docs/tasks#t210120153618) compression
...
Notes:
- This feature is *not* currently [implemented](http://github.com/galliaproject/gallia-docs/tasks#t210204111309), although I have some PoC code available - ideally this would be an alternative run mode for _Spark_ itself
...
- We can combine such operations and leverage named pipes to ensure an execution tree can be executed lazily (forking however would benefit from a form of [checkpointing](http://github.com/galliaproject/gallia-docs/tasks#t210121160956)
...
I am providing a [link](http://github.com/galliaproject/gallia-dbnsfp) to one of the full blow examples I've written using _Gallia_: turning the big
...
a prominent place in the task [list](http://github.com/galliaproject/gallia-docs/tasks#t210121095401.)
...
### How can I help?
I'm already aware of many issues and have a long list of [tasks](http://github.com/galliaproject/gallia-docs/tasks#t210121100050) meant to address them, as well as add the features that are critically missing.
...
On the batch ETL front, it would be interesting to see how alternative libraries/languages take examples such as the [dbNSFP](#210121135252) one above.
...
### What about features like streaming? EDA? [visualization](http://github.com/galliaproject/gallia-docs/tasks#t210124092211)? linear algebra? graph queries? notebooks? metadata [semantics](http://github.com/galliaproject/gallia-docs/tasks#t210124095616) ? squaring the circle?
...
I haven't acquainted myself with 2.13 yet, let alone Dotty. Corresponding support is [planned](http://github.com/galliaproject/gallia-docs/tasks#t210121101237) however.
...
I see them helping a lot in particular with boilerplate and some compile-time validation (e.g. key [validation](http://github.com/galliaproject/gallia-docs/tasks#t210127134525)).
...
As the project matures, proper scaladoc-friendly comments can hopefully be [added](http://github.com/galliaproject/gallia-docs/quotes) as well.
