...
class DAG[$NodeType](
      ...
      extends Serializable /* some Atoms use sub-plans, meaning we need to be able to serialize the DAG */ {
    ...
    @transient              lazy val nodeIds  : Seq[NodeId] = nodes.map(idResolver) // used in Handler's "join2"
    @transient private[dag] lazy val nodeIdSet: Set[NodeId] = nodeIds.toSet
    ...
    def formatDefault2(f: $NodeType => String) =        
      ...
      def formatDot: String    = ???//graphviz.DAGDot.formatIdDot(toIdDAG)
      def  printDot: this.type = { formatDot.p; this }

      // ---------------------------------------------------------------------------
      def formatDot(lookup: $NodeType => (String /* label */, String /* color */)): String    = ???//graphviz.DAGDot.formatDot(this)(lookup)
    ...
    // ===========================================================================
    @transient private[dag] lazy val _efferents: Map[NodeId, Nes[NodeId]] = edges.groupBy(_._1).mapValues(_.map(_._2)).toMap
    @transient private[dag] lazy val _afferents: Map[NodeId, Nes[NodeId]] = edges.groupBy(_._2).mapValues(_.map(_._1)).toMap
    ...
    @transient lazy val kahnTraverseNodes: Seq[$NodeType] = kahnTraverseIds.map(_lookup)

    def chainTraverseNodes: Seq[$NodeType] = ChainUtils.chainTraversal(this)(this.rootIds)
            
    // ---------------------------------------------------------------------------
    @transient lazy val roots : Seq[$NodeType] = nodeIds.diff(_afferents.keys.toSeq).map(_lookup)
    @transient lazy val leaves: Seq[$NodeType] = nodeIds.diff(_efferents.keys.toSeq).map(_lookup)

    @transient lazy val rootIds : Seq[NodeId] = roots .map(idResolver)
    @transient lazy val leaveIds: Seq[NodeId] = leaves.map(idResolver)
    ...
    @transient lazy val isChain: Boolean = 
