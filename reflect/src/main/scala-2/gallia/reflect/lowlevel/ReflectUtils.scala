...
private object ReflectUtils {
  ...
  private[reflect] def parseFields(tpe: UType): List[(String, UType)] =
    ...
    // ---------------------------------------------------------------------------
    private[reflect] def methodSymbols(tpe: runiverse.Type) = // can't easily refactor with above, so at least keep them together at least
      tpe
        .decls
        .filter((x: api.Symbols#SymbolApi) => x.isMethod)
        .map   (_.asMethod)
  ...
  // ---------------------------------------------------------------------------
  private[reflect] def fieldTpes(tpe: runiverse.Type): Seq[runiverse.Type] = // can't easily refactor with above, so at least keep them together at least
