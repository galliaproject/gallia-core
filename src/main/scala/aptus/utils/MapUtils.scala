...
object MapUtils {
  ...
  def groupByKey[K, V](entries: Seq[(K, V)]): immutable.Map[K, Seq[V]] = {
      val m = mutable.Map.empty[K, mutable.MutableList[V]]

      for (elem <- entries) {
        val key = elem._1
        val bldr = m.getOrElseUpdate(key, mutable.MutableList[V]())
        bldr += elem._2
      }
    ...
    // ---------------------------------------------------------------------------
    def groupByKeyWithListMap[K, V](entries: Seq[(K, V)]): immutable.ListMap[K, Seq[V]] = {
      var m = immutable.ListMap.empty[K, mutable.MutableList[V]]

      for (elem <- entries) {
        val key = elem._1
        val bldr =
          m.get(key) match { //m.getOrElseUpdate(key, mutable.MutableList[V]())
            case Some(x) => x
            case None =>
              val x = mutable.MutableList[V]()
    ...
    // ---------------------------------------------------------------------------
    def groupByKeyWithTreeMap[K, V](entries: Seq[(K, V)])(implicit ord: Ordering[K]): immutable.TreeMap[K, Seq[V]] = {
      val m = mutable.TreeMap.empty[K, mutable.MutableList[V]]
