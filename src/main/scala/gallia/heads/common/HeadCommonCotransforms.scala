...
trait HeadCommonCotransforms[F <: HeadCommon[F]] { ignored: HeadCommon[F] => // TODO: t210826102833 - rework co-transforms
  ...
  def cotransform[O1: WTT](f1: Cotransform[O1]) = new { import TSL.Cotransform._
      ...
      def asOverwrite = new { def using[D1: WTT](f: O1 => D1): Self2 =
        self2 :+ Cotransform1to1(tmp, Left(HT.parse[D1]), wrap11(f) ) }

      def as(a: KPathW)                       = new { def using[D1: WTT                  ](f: O1 =>  D1         ): Self2 = self2 :+ Cotransform1to1(tmp, ttqkpath1[D1        ](a      ).in.right, wrap11(f)) }
    ...
    def cotransform[O1: WTT, O2: WTT](f1: Cotransform[O1], f2: Cotransform[O2]) = new { import TSL.Cotransform._
      ...
      def asOverwrite = new { def using[D1: WTT, D2: WTT](f: (O1, O2) => (D1, D2)): Self2 =
        self2 :+ Cotransform2to2(tmp, HT2.from[D1, D2].in.left, wrap22(f) ) }

      def as(a: KPathW)                       = new { def using[D1: WTT                  ](f: (O1, O2) =>  D1         ) = self2 :+ Cotransform2to1(tmp, ttqkpath1[D1        ](a      )         , wrap21(f)) }
      def as(a: KPathW, b: KPathW)            = new { def using[D1: WTT, D2: WTT         ](f: (O1, O2) => (D1, D2    )) = self2 :+ Cotransform2to2(tmp, ttqkpath2[D1, D2    ](a, b   ).in.right, wrap22(f)) }
