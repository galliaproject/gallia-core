...
trait HeadZGrouping { self: HeadZ =>
  ...
  // ===========================================================================
  def grouping(conf: Start => End): Self = zz(GroupData.from(conf).pipe(GroupingAction))

      private def hasAs(conf: Start => End): Self with HasAs = zzWithAs(GroupData.from(conf).pipe(GroupingAction))
