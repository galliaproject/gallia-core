...
trait ClsLike { // read-only part
    ...
    // ---------------------------------------------------------------------------
    def complementKeyz(x: Key ): Keyz = keys.filterNot(_ == x).thn(Keyz.apply) //TODO: ensure match...
    def complementKeyz(x: Keyz): Keyz = keys.diff(x.values)   .thn(Keyz.apply) //TODO: ensure match...
