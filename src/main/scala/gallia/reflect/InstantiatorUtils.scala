...
object InstantiatorUtils {
  ...
  // ===========================================================================
  def rec[$Instantiator](constr: InstantiatorContructor[$Instantiator])(mirror: universe.Mirror)(tpe: universe.Type): $Instantiator = {
        ...
        .flatMap { case (methodSymbol, f) =>
          if (!f.node.isContainedDataClass) None
            ...
            methodSymbol
              .typeSignature
              .resultType
              .pipe(subInstantiator(constr)(f.node.containerType, mirror, _))
