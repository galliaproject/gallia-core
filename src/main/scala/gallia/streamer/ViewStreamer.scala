...
//override def toView  : ViewRepr[A] = view
  ...
  override def toMeBased [B >: A : WTT](that: Streamer[B]): Streamer[B] = that.toViewBased

  // ===========================================================================
  override def     map[B: WTT](f: A =>      B ): Streamer[B] = view.    map(f).toSeq.view.pipe(_rewrap)
  override def flatMap[B: WTT](f: A => Coll[B]): Streamer[B] = view.flatMap(f).toSeq.view.pipe(_rewrap)
  ...
  // ===========================================================================
  override def union[B >: A : WTT](that: Streamer[B])                       : Streamer[B] = _utils.union(this.asInstanceOf[ViewStreamer[B]], that)
  override def zip  [B >: A : WTT](that: Streamer[B], combiner: (B, B) => B): Streamer[B] = _utils.zip  (this.asInstanceOf[ViewStreamer[B]], that, combiner)
