...
object GalliaPackageTest extends utest.TestSuite {
 import utest._
  ...
  val tests = Tests {
    ...
    test { assert(gallia.aobjFromDataClass(TestMeta.Foo("x", "y"))._forceResult.c.formatCompactJson == """{"fields":[{"key":"a","info":{"optional":false,"union":[{"multiple":false,"valueType":"_String"}]}},{"key":"A","info":{"optional":false,"union":[{"multiple":false,"valueType":"_String"}]}}]}""") }
    test { assert(gallia.aobjFromDataClass(TestMeta.Foo("x", "y"))._forceResult.o.formatCompactJson == """{"a":"x","A":"y"}""") }
  }
